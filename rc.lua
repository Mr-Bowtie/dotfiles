...
globalkeys = my_table.join(
    ...
    awful.key({ altkey, "Control" }, "0",
        ...
        -- changes output device to yeti (headphones)
    awful.key({ altkey, "Control" }, "h",
        function()
            os.execute(string.format("pactl set-default-sink alsa_output.usb-Blue_Microphones_Yeti_Stereo_Microphone_797_2018_04_29_48753-00.analog-stereo"))
        end),
    -- changes output to computer audio (HDMI)
    awful.key({ altkey, "Control" }, "c",
        function()
            os.execute(string.format("pactl set-default-sink alsa_output.pci-0000_01_00.1.hdmi-stereo-extra2"))
        end),
    -- changes output to tower audio
...
--]]
    ...
    -- Discord
    awful.key({ modkey, "Mod1"}, "d", function() awful.util.spawn("discord") end,
              {description = "run discord", group = "launcher"}),

    -- Pulseaudio Volume Control  	
    awful.key({ modkey, "Mod1"}, "p", function() awful.util.spawn("pavucontrol") end,
              {description = "Launch Pulseaudio GUI", group = "launcher"}),
    ...
    -- Steam
    awful.key({ modkey, "Mod1"}, "e", function() awful.util.spawn("steam") end,
    ...
    awful.key({ modkey, "Mod1"}, "n", function()
        awful.util.spawn("notion-app")
        awful.util.spawn("notion-app")
        awful.tag.incmwfact( -0.25)
    end,
              {description = "Launch Notion", group = "launcher"}
    ),
    
    -- Mousepad
    awful.key({ modkey, "Mod1"}, "m", function() awful.util.spawn("mousepad") end,
              {description = "Launch Mousepad", group = "launcher"}
    ),
    ...
    --]]
    -- Prompt
    awful.key({ modkey }, "r", function () awful.screen.focused().mypromptbox:run("dmenu") end,
...
for i = 1, 9 do
    ...
    globalkeys = my_table.join(globalkeys,
        ...
        awful.key({ modkey, "Control" }, "#" .. i + 9,
                  ...
                  descr_toggle),
        -- View tag on all screens (3 screen layout) *******CUSTOM********
        awful.key({ modkey, "Mod1", "Control" }, "#" .. i + 9,
                 ...
                 {description = "view (on all screens) tag #" .. i, group = "tag"}
        ),
        -- Move client to tag.
...
-- awful.util.spawn("fish -c startup") 
